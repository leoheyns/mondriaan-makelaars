...
var pgSelectorCache = {};
var pgDontFormat = ['p', 'span', 'bdo', 'em', 'strong', 'dfn', 'code', 'samp', 'kbd', 'var', 'cite', 'abbr', 'acronym', 'q', 'sub', 'sup', 'tt', 'i', 'b', 'big', 'small', 'u', 's', 'strike', 'font', 'ins', 'del', 'pre', 'address', 'dt', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'br'];//, '?php', '?='];
...
var pgScriptsTagsRegExp = null;
var pgScriptsStartsWithRegExp = null;
var pgScriptsStartsWithCloseRegExp = null;

var pgAttrScripts = [];//[['<?php', '?>'], ['<?=', '?>']];
...
var pgPrepareScripts = function() {
    ...
    for (var key in pgScripts) {
        if (pgScripts.hasOwnProperty(key)) {
            var def = pgScripts[key];
            def.open = key;
            pgScriptsList.push(def);

            var tag = def.open.replace('<', '');
            pgScriptsByTag[tag] = def;

            pgDontFormat.push(tag);
            pgSingleTags.push(tag);

            re += (re.length ? '|' : '') + '(' + escapeRegExp(def.open) + ')';
            re_tags += (re_tags.length ? '|' : '') + '(' + escapeRegExp(tag) + ')';
            re_sw += (re_sw.length !== 1 ? '|' : '') + '(' + escapeRegExp(def.open) + ')';
            re_sw_close += (re_sw_close.length !== 1 ? '|' : '') + '(' + escapeRegExp(def.close) + ')';

            pgAttrScripts.push([key, def.close]);
        }
    }


    pgScriptsRegExp = new RegExp(re, 'i');
    pgScriptsTagsRegExp = new RegExp(re_tags, 'i');
    pgScriptsStartsWithRegExp = new RegExp(re_sw, 'ig');
    pgScriptsStartsWithCloseRegExp = new RegExp(re_sw_close, 'ig');
}
...
var pgIsScriptTag = function(tag) {
    return tag.match(pgScriptsTagsRegExp);
}

var pgGetScriptInfo = function(opening_tag) {
    if(opening_tag[0] != '<') return null;
    return pgScripts[opening_tag] || null;
}

var pgGetScriptInfoByTag = function(tag) {
    return pgScriptsByTag[tag] || null;
}

var pgTestAtIndex = function(str, re, i) {
    re.lastIndex = i;
    var m = re.test(str);
    return m;
}

var pgMatchAtIndex = function(str, re, i) {
    re.lastIndex = i;
    var m = re.exec(str);
    return m;
}
...
var startsWithCaseInsensitiveScript = function(source, position) {
    for(var i = 0; i < pgScriptsList.length; i++) {
        if(startsWithCaseInsensitive(pgScriptsList[i].open, source, position)) {
            return pgScriptsList[i];
        }
    }
    return null;
}
...
//    if(this.script && (this.tagName == '?php' || this.tagName == '?=')) return true;
    if(this.script && pgIsScriptTag(this.tagName)) return true;
...
pgParserNode.prototype.makeListByWalkingSelfAndChildren = function() {
    var list = [];
    this.walkSelfAndChildren(function(node) {
        if(node.isElement) list.push(node);
        return true;
    })
    return list;
}
...
pgParserNode.prototype.findOneWithAttrValue = function(attr, value, tag) {
    tag = tag || '';
    var list = this.find(tag + '[' + attr + ']');
    for(var i = 0; i < list.length; i++) {
...
pgParserNode.prototype.findWithAttrValue = function(attr, value, tag) {
    tag = tag || '';
    var r = [];
    var list = this.find(tag + '[' + attr + ']');
    for(var i = 0; i < list.length; i++) {
...
pgParserNode.prototype.isSelector = function(sel, parsed_sel) {
    if(!parsed_sel) {
        if(sel.indexOf(',') >= 0) {
            //handle multiple , separated selectors
            var list = crsaSplitAndTrimList(sel);
            for(var i = 0; i < list.length; i++) {
                if(this.isSelector(list[i])) return true;
            }
            return false;
        }
    }
...
pgParserNode.prototype.closest = function(sel, stop_at) {
    var _this = this;
    var sel_list = sel.split(',');
    var parsed_sels = sel_list.map(function(item) { return _this.parseSelector(item) });;
    var r = this;

    var isAny = function() {
        for(var i = 0; i < sel_list.length; i++) {
            if(r.isSelector(sel_list[i], parsed_sels[i])) return true;
        }
        return false;
    }
...
//    while(!r.isSelector(sel, parsed_sel)) {
    while(!isAny()) {
        ...
        if(stop_at && stop_at == r) return null;
        r = r.parent;
    }
    return r;
}
...
pgParserNode.prototype.clone = function(dont_asign_id, copy_ids) {
    ...
    node.script_info = this.script_info;
...
pgParserNode.prototype.hasChildren = function(all) {
    for(var i = 0; i < this.children.length; i++) {
        if(this.children[i].isElement || all) {
            return true;
        }
    }
    return false;
...
pgParserNode.prototype.setInlineStylePropertyValue = function(prop, value, is_url) {
    var style = crsaSetInlineStylePropertyValue(this.getAttr('style'), prop, value, is_url);
    if(style) {
...
pgParserNode.prototype.changeToPhp = function(code) {
    ...
    this.script = true;
    this.script_info = pgScripts['<?php'];
    this.isElement = false;
    this.textNode = false;
    this.singleTag = true;
    this.closed = true;
...
*/
    ...
    if(pgIdAdd && pgIsScriptTag(this.tagName)) {
 ...
 //           if(this.content.indexOf("\n") >= 0) return '\n';
    ...
    for(var i = 0; i < this.children.length; i++) {
        ...
        if(withIds && pgIsScriptTag(this.children[i].tagName) && php_ids) {
...
var pgParser = function() {
    ...
    var tokenGetTagName = function(ch, s, ch_code) {
        ...
        if(tokenGetTagNameState === 2) {
            return false; //prev char was = or %, this could be <%miskolin
        }
        if(tokenGetTagNameState === 0) {
            if(ch_code === CHAR_EXCLAMATION /* '!' */) {
                tokenGetTagNameState = 1;
            } else if(ch_code === CHAR_EQUAL || ch_code === CHAR_PERCENT) {
                tokenGetTagNameState = 2; //next char will be false - ends script tags like <%miskolin %>
                return true;
            }
        }
            ...
            while(i < len) {
                ...
                if(idx < 0) {
                   ...
                   // } else if(tagLower === '?php' || tagLower === '?=') {
                    } else if(si = pgGetScriptInfoByTag(tagLower)) {
                        attrs = readUntilIncluding(si.close);
                        if(attrs.length && attrs.endsWith(si.close[0])) {
                    ...
                    if(tag.charCodeAt(0) !== CHAR_SLASH /* '/' */) {
                        ...
                        //si = pgGetScriptInfoByTag(tagLower);
                        if(si) {
