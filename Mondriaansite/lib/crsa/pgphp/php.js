  ...
  CodeMirror.defineMode("php", function(config, parserConfig) {
    ...
    function dispatch(stream, state) {
      ...
      if (!isPHP) {
        ...
        var cur = stream.current(), openPHP = cur.search(/<(\?|php)/), m;
        if (openPHP != -1) {
          if (style == "string" && (m = cur.match(/[\'\"]$/)) && !/(\?|<\/php)>/.test(cur)) state.pending = m[0];
          else state.pending = {end: stream.pos, style: style};
          stream.backUp(cur.length - openPHP);
        }
        return style;
      } else if (isPHP && state.php.tokenize == null && stream.match(/(\?|<\/php)>/)) {
    ...
    return {
      ...
      indent: function(state, textAfter) {
        if ((state.curMode != phpMode && /^\s*<\//.test(textAfter)) ||
            (state.curMode == phpMode && /^(\?|<\/php)>/.test(textAfter)))
